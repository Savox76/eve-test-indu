...
**Technische Preview – `v0.0.5-preview.8`.** Assets, Blueprints, persönliche Industrieaufträge und die vollständigen Skillstände aller aktivierten Charaktere werden beim Programmstart charaktergetrennt synchronisiert. Die echten Ansichten sind durchsuchbar, filterbar und sortierbar; abgeschlossene Jobs werden nachvollziehbar mit Blueprint-Snapshots und passenden Asset-Änderungen korreliert. Trainiertes und aktuell wirksames Skilllevel sowie verteilte und freie Skillpunkte bilden die belegbare Grundlage für spätere Machbarkeits- und Lückenprüfungen. Nur vollständige Läufe werden als gültige Snapshots veröffentlicht. Jede EVE-Anmeldung fordert automatisch alle aktuell benötigten SSO-Pakete an, während bestehende Charaktere bei späterer Scope-Erweiterung erhalten bleiben und sichtbar zur erneuten Anmeldung auffordern.

Charakteridentitäten, Aliasse, Gruppen und Scope-Status stammen weiterhin aus SQLite; Refresh Tokens bleiben im Windows-Anmeldespeicher und Access Tokens ausschließlich im Sidecar-Prozess. Manuelle Folgeläufe bleiben in den Asset-, Blueprint-, Job- und Skill-Arbeitsbereichen verfügbar. Installierte Ausgaben speichern ihre Daten updatefest im benutzerspezifischen App-Datenordner; portable Ausgaben behalten den sichtbaren `data`-Ordner im konstanten Verzeichnis `New Eden Foundry Portable`. `v0.0.5-preview.8` ist der aktuelle Kandidat für die [verbindliche Windows-A0-Abnahme](docs/windows-a0-acceptance.md).

Registrierungswerte stehen im [verbindlichen Registrierungsprofil](docs/sso-registration.md); die Betriebsgrenzen beschreiben die [PKCE-Dokumentation](docs/sso-pkce-login.md), die [Tokenprüfung](docs/sso-token-validation.md), die [Tokenablage](docs/token-security.md), die [Charakterverwaltung](docs/character-management.md), der [zentrale ESI-Client](docs/esi-client.md), der [Asset-Sync](docs/asset-sync.md), die [Standortauflösung](docs/location-resolution.md), die [Asset-Oberfläche mit CSV-Export](docs/asset-ui.md), die [Asset-Deltas](docs/asset-deltas.md), der [Blueprint-Bestand](docs/blueprint-inventory.md), die [Industrieaufträge](docs/industry-jobs.md) und die [Charakter-Skills](docs/character-skills.md). Die Anwendung enthält kein Client Secret und gibt weder Tokens noch `state`, Verifier oder Autorisierungscode an Oberfläche oder Logs weiter.
