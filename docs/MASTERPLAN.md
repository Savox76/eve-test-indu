...
- **In Arbeit:** 22 – Der Gerätetest bestätigte Installer, Betrieb ohne Administratorrechte, Einzelinstanz, sauberes Prozessende und Release-Erkennung. `v0.0.5-preview.16` repariert den durch eine gesperrte Zusatzdatei blockierten `.15`-Start, erhält den sichtbaren `data`-Ordner neben Installer- und Portable-EXE und ergänzt einen echten installierten CI-Starttest. Das vollständige A0-Protokoll bleibt vor der ersten `0.2.0`-Alpha erforderlich.
