...
Diese Abnahme ist der letzte externe Nachweis vor der ersten `0.2.0`-Alpha. Sie wird auf einem freigegebenen Windows-10- oder Windows-11-x64-Testgerät mit einer vorhandenen Preview und dem Korrekturkandidaten `v0.0.5-preview.26` durchgeführt. Der CI-Pakettest scannt Sidecar, Installer und Portable-ZIP mit Microsoft Defender, verlangt für die Haupt-EXE das Windows-GUI-Subsystem, installiert den Kandidaten, beendet den Sidecar einmal absichtlich, verlangt seine automatische Wiederherstellung, legt einen frischen Cache ohne eigenes Ablaufdatum an, schließt die Anwendung normal und startet denselben Datenstand erneut; den vollständigen Gerätetest ersetzt er dennoch nicht.
...
| ---: | --- | --- |
| 1 | Installation `v0.0.5-preview.26` | Der Current-User-Installer beendet sich erfolgreich und die Anwendung startet ohne Entwicklerwerkzeuge. Es erscheint nur die grafische Übersicht und kein zusätzliches Konsolenfenster. |
...
| 5 | Installer-Update | Nach vollständigem Schließen wird `v0.0.5-preview.26` über `.16`, `.17`, `.19`, `.20`, `.21`, `.22`, `.23`, `.24` oder `.25` installiert. Die Datenlöschung bleibt abgewählt; anschließend sind Version, Charaktere, Einstellungen, Pläne und Snapshots vorhanden und `data` liegt neben der EXE. |
