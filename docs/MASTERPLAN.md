...
- **In Arbeit:** 22 – Der Gerätetest bestätigte Einzelinstanz, zweiten Start und updatefesten Speicher. `v0.0.5-preview.13` ist der aktuelle technische Kandidat. Das vollständige A0-Protokoll bleibt vor der ersten `0.2.0`-Alpha erforderlich.
...
- **Abgeschlossen:** 30 – persistente Fertigungs- und Reaktionsziele werden aus dem exakten Wurzelrezept und einer stabilen Zwischenprodukt-Auswahl deterministisch aufgelöst. Gemeinsamer Bedarf wird vor ganzzahliger Laufberechnung aggregiert; Schritte, Überschuss, SDE-Basiszeit, äußeres Bruttomaterial, Alternativen, Zyklen und fehlende SDE-Stände bleiben nachvollziehbar. Schema 9 speichert Zielmenge, Priorität und Notiz updatefest. Die Slotübersicht zählt passende Ziele als geplant, laufend oder blockiert, ohne aus Bestand einen Fertigzustand zu erfinden.
- **Als Nächstes:** Das offene Paket 22 wird mit dem vollständigen Windows-A0-Protokoll für `v0.0.5-preview.13` abgeschlossen. Die bereits technisch vorgezogenen Fachpakete 23–30 bleiben Previews; eine erste `0.2.0`-Alpha wird erst nach bestandenem A0 freigegeben. Bestand, Reservierung und belastbare Zeit-/Kostenmodifikatoren folgen erst nach eigenen Fachverträgen und werden nicht vorweggenommen.
