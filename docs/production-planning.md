...
## Zielbezogene Einkaufsliste

Paket 48 bildet die Einkaufsliste aus `missingQuantity` für genau das in der Oberfläche ausgewählte Produktionsziel. Der vollständige Bestandsabgleich und die globale Reservierungsreihenfolge werden weiterhin vor Suche, Filtern, Sortierung und Seitenauswahl über alle Produktionsziele berechnet. Dadurch bleibt eine durch ein vorrangiges anderes Ziel verursachte `reservationConflictQuantity` korrekt sichtbar, ohne dessen eigene Materialien in die Einkaufsliste zu mischen. Ein Zielwechsel berechnet Einkauf, Kapitalbedarf, Verkaufserlös und Gewinn gemeinsam neu.

Die Ausgabe ist nur dann `ready`, wenn das ausgewählte Ziel ein auflösbares Rezept und einen vollständigen Asset-Snapshot besitzt. `empty` bedeutet ein vollständig belegtes Ziel ohne Fehlmengen oder noch kein ausgewähltes Ziel. `incomplete` markiert ein nicht auflösbares ausgewähltes Ziel; ein fehlender Snapshot oder ein blockiertes Rezept wird niemals als Fehlmenge null ausgelegt. Bis zu 1.000 Materialarten werden übertragen. Wird diese Grenze überschritten, nennt die Antwort die ausgelassenen Typen und deaktiviert das Kopieren, statt eine gekürzte Liste als vollständig auszugeben.
...
Paket 45 trennt den zusätzlich benötigten Kapitalbedarf von der wirtschaftlichen Bewertung. Bereits vorhandener Bestand reduziert den Sofortkauf, wird für die Rohmarge jedoch nicht mit null angesetzt. Paket 48 wendet diese Bewertung ausschließlich auf alle äußeren Bruttomaterialien des ausgewählten Ziels an und bewertet sie aus der stationsexakten Sell-Order-Tiefe vollständig zum Wiederbeschaffungswert.
...
## Blueprint-Rentabilität je Handelsstation

Paket 48 verwendet ausschließlich konfigurierte Produktionsziele als belastbare Vergleichseinheit. Dadurch fließen Zielmenge, konkrete Blueprintzuordnung und ME, Versorgungsmodi, Bestandsreservierungen, Anlagenprofil, Installationskosten sowie Verkäufer-Skills und stationsbezogene Standings unverändert in jede Stationsspalte ein. Eine pauschale Ein-Lauf-Schätzung für einen Blueprint ohne Zielkonfiguration wird bewusst nicht erfunden.

Für jedes gefilterte Ziel wird dieselbe exakte Auflösung getrennt gegen Jita, Amarr, Dodixie, Hek und Rens bewertet. Jede Zelle zeigt Nettogewinn oder -verlust und Nettomarge; `bestHubId` bezeichnet den höchsten vollständig berechenbaren Nettogewinn, auch wenn alle Hubs negativ sind. Fehlende Markt-, Skill-, Standing- oder Installationsbelege bleiben unbekannt. Höchstens 100 Ziele und 250 eindeutige Markt-Typen werden je Vergleich übertragen; Überschreitungen werden als ausgelassene Ziele oder Typen kenntlich gemacht.

Die Blueprint-Ansicht kann die benötigten Typen für alle fünf Hubs bewusst gemeinsam aktualisieren. Die einzelnen Hub-Synchronisationen bleiben atomar und unabhängig: Ein Fehler ersetzt keinen älteren vollständigen Snapshot. Der Vergleich ist eine Entscheidungsgrundlage zum niedrigsten aktuellen Sell-Angebot, keine Ausführungs- oder Absatzgarantie.
