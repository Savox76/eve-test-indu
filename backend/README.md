...
Der Sidecar bündelt außerdem das vollständige öffentliche EVE-SSO-Registrierungsprofil und meldet dessen Zustand als `registered`. Das Profil enthält die öffentliche Client-ID und ausdrücklich kein Client Secret. Für jeden Anmeldeversuch erzeugt er unabhängige 256-Bit-Werte für `state` und PKCE-Verifier, bindet kurzzeitig den festen Callback `127.0.0.1:17891`, prüft den Rückruf exakt und unterstützt Status, Drei-Minuten-Timeout und Abbruch über die authentifizierte interne API. In Paket 13 tauscht er den Code mit PKCE aus, bezieht Token- und JWKS-Endpunkte aus streng geprüften EVE-Metadaten und validiert `RS256`-Signatur, Schlüssel-ID, Issuer, beide Audience-Werte, Ablauf, Charakter-ID, Name und Scopes, bevor die Identität in SQLite gespeichert wird.

EVE autorisiert jeden Charakter einzeln. Eine lokale Kontogruppe dient nur der vom Nutzer vergebenen Organisation mehrerer Charaktere; sie speichert weder EVE-Accountnamen noch Zugangsdaten. Token werden in Paket 13 nach der Identitätsprüfung noch verworfen; die spätere dauerhafte Ablage erfolgt pro Charakter im Windows-Anmeldespeicher und niemals in SQLite. Die globale Schriftgrößenstufe wird als nicht geheime Einstellung in der bestehenden Tabelle `app_settings` gespeichert.
