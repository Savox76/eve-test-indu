...
**Technische Preview – `v0.0.5-preview.6`.** Assets und Blueprints aller aktivierten Charaktere werden beim Programmstart vollständig und charaktergetrennt synchronisiert. Die echten Asset- und BPO/BPC-Ansichten sind durchsuchbar, filterbar und sortierbar; nur vollständig abgeschlossene Läufe werden als gültige Snapshots veröffentlicht. Jede EVE-Anmeldung fordert automatisch alle aktuell benötigten SSO-Pakete an, während bestehende Charaktere bei späterer Scope-Erweiterung erhalten bleiben und sichtbar zur erneuten Anmeldung auffordern.

Charakteridentitäten, Aliasse, Gruppen und Scope-Status stammen weiterhin aus SQLite; Refresh Tokens bleiben im Windows-Anmeldespeicher und Access Tokens ausschließlich im Sidecar-Prozess. Manuelle Folgeläufe bleiben in den Asset- und Blueprint-Arbeitsbereichen verfügbar. Installierte Ausgaben speichern ihre Daten updatefest im benutzerspezifischen App-Datenordner; portable Ausgaben behalten den sichtbaren `data`-Ordner im konstanten Verzeichnis `New Eden Foundry Portable`. `v0.0.5-preview.6` ist der aktuelle Kandidat für die [verbindliche Windows-A0-Abnahme](docs/windows-a0-acceptance.md).
