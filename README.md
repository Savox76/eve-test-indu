...
**Technische Preview – `v0.0.4-preview.6`.** Verbundene EVE-Charaktere lassen sich jetzt vollständig lokal verwalten: Alias, Aktivstatus und frei benannte Kontogruppen sind editierbar; Credential- und Scopepaket-Status bleiben pro Charakter sichtbar. Das bewusste Löschen ist zweistufig bestätigt und entfernt Identität, Scopes, Synchronisierungshistorie, Cache-Snapshots, Prozess-Token und den Refresh Token.

Charakteridentitäten, Aliasse, Gruppen und Scope-Status stammen aus SQLite; Geheimnisse werden dort nie abgelegt. Die Fachwerte der Mehrcharakter-Vorschau bleiben klar synthetisch. Der zentrale ESI-Client, SDE, echte Synchronisierung und fachliche Berechnungen sind noch nicht angeschlossen. Eine neue Installation meldet korrekt, dass noch keine lokalen Fachdaten vorhanden sind. Versionen werden weiterhin manuell über [GitHub Releases](https://github.com/Savox76/eve-test-indu/releases) bezogen; das vollständige Windows-Installationsgate bleibt Voraussetzung für die erste technische Alpha.

Registrierungswerte stehen im [verbindlichen Registrierungsprofil](docs/sso-registration.md); Ablauf und Sicherheitsgrenze beschreiben die [PKCE-Betriebsdokumentation](docs/sso-pkce-login.md), die [Token-/Charakterprüfung](docs/sso-token-validation.md), die [Tokenablage mit Rotation](docs/token-security.md) und die [Charakterverwaltung](docs/character-management.md). Die Anwendung enthält kein Client Secret und gibt weder Tokens noch `state`, Verifier oder Autorisierungscode an Oberfläche oder Logs weiter. Die Bedienung der Schriftstufen beschreibt [appearance.md](docs/appearance.md).
