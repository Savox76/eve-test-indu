...
Diese Abnahme ist der letzte externe Nachweis vor der ersten `0.2.0`-Alpha. Sie wird auf einem freigegebenen Windows-10- oder Windows-11-x64-Testgerät mit dem Installer von `v0.0.5-preview.2` und dem Kandidaten `v0.0.5-preview.11` durchgeführt. Automatisierte CI-Pakettests ersetzen diesen Lauf nicht.
...
| 5 | Installer-Update | Nach vollständigem Schließen wird `v0.0.5-preview.11` über dieselbe Installation installiert und zeigt anschließend die neue Versionsnummer. |
