...
- **In Arbeit:** 22 – Der Gerätetest bestätigte Installer, Betrieb ohne Administratorrechte, Einzelinstanz und Release-Erkennung. `v0.0.5-preview.17` schließt die nach `.16` verbliebene Neustartlücke: Der lokale Dienst erhält beim normalen Beenden ein geschlossenes Steuerrohr, kurzzeitige Neustartfehler werden begrenzt wiederholt und der Windows-CI-Test verlangt nun ausdrücklich Schließen, vollständiges Prozessende und einen zweiten erfolgreichen Start mit demselben `data`-Ordner. Das vollständige A0-Protokoll bleibt vor der ersten `0.2.0`-Alpha erforderlich.
