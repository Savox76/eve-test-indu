...
**Technische Preview – `v0.0.5-preview.14`.** Assets, Blueprints, persönliche Industrieaufträge und die vollständigen Skillstände aller aktivierten Charaktere werden beim Programmstart charaktergetrennt synchronisiert. Anlagen, Systemkosten, Forschungsplanung und Industrie-Slots verbinden diese vollständigen Quellen ohne unbekannte Werte als null zu schätzen. Paket 30 ergänzt persistente Fertigungs- und Reaktionsziele: Die App installiert den geprüften offiziellen SDE-Produktionskatalog automatisch und löst gewählte Rezepte deterministisch in Produktionsschritte, ganzzahlig gerundete Läufe, Überschuss, Basiszeit und äußeren Bruttomaterialbedarf auf. Bestand, Reservierungen, Blueprint-ME, Charakter-, Anlagen- und Rigmodifikatoren, Steuern und Preise bleiben dabei ausdrücklich unberücksichtigt. Alle echten Ansichten sind zweisprachig, durchsuchbar, filterbar und begrenzt.

Charakteridentitäten, Aliasse, Gruppen, Scope-Status, Forschungs- und Produktionspläne stammen aus SQLite; Refresh Tokens bleiben im Windows-Anmeldespeicher und Access Tokens ausschließlich im Sidecar-Prozess. Die App prüft automatisch und auf Knopfdruck, ob im gewählten Kanal ein vollständiges neueres GitHub Release bereitsteht, und zeigt den passenden Installer- oder Portable-Hinweis. Download und Installation bleiben bis zu einer produktiv signierten Updatekette bewusst manuell. `v0.0.5-preview.14` ist der aktuelle Kandidat für die [verbindliche Windows-A0-Abnahme](docs/windows-a0-acceptance.md).
...
Die portable ZIP enthält Hauptprogramm und Sidecar im konstanten Ordner `New Eden Foundry Portable` und benötigt die Microsoft Edge WebView2 Runtime. „Portable“ bedeutet hier einen vollständig entpackbaren, verschiebbaren Programmordner – nicht den Start direkt im ZIP und nicht eine einzelne EXE. Beim ersten Start entsteht dort `data\foundry.sqlite3`. Für ein Update die App schließen, `data` zusätzlich sichern und die neue ZIP in denselben übergeordneten Ordner entpacken; ausgelieferte Programmdateien werden ersetzt, der nicht im ZIP enthaltene `data`-Ordner bleibt erhalten. Die App erkennt portable Ausgaben und zeigt diesen Ablauf zusammen mit einem Link zum neuesten vollständigen Release an. EVE-Refresh-Tokens bleiben getrennt im Windows-Anmeldespeicher. Zu jeder Variante gehört eine gleichnamige `.sha256`-Datei.
