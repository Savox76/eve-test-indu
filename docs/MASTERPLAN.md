...
- **In Arbeit:** 22 – Der Gerätetest bestätigte Installer, Betrieb ohne Administratorrechte, Einzelinstanz und Release-Erkennung. `v0.0.5-preview.18` enthält die Neustartkorrektur aus `.17` sowie die Abnahmenacharbeit für Assets, Blueprints, Jobs, Anlagen und Produktionsziele. Der Windows-CI-Test verlangt Schließen, vollständiges Prozessende und einen zweiten erfolgreichen Start mit demselben `data`-Ordner. Das vollständige A0-Protokoll bleibt vor der ersten `0.2.0`-Alpha erforderlich.
