...
- **In Arbeit:** 22 – Der Gerätetest bestätigte Installer, Betrieb ohne Administratorrechte, Einzelinstanz und Release-Erkennung. `v0.0.5-preview.20` enthält die Neustartkorrektur aus `.17`, die Abnahmenacharbeit aus `.18`, die Defender-geprüfte Sidecar-Paketierung aus `.19` sowie die automatische Wiederherstellung eines unerwartet beendeten Sidecars. Der Windows-CI-Test beendet den Sidecar absichtlich, verlangt einen neuen Prozess, bricht verwaiste Sync-Läufe sicher ab und prüft danach Schließen, vollständiges Prozessende und einen zweiten erfolgreichen Start mit demselben `data`-Ordner. Das vollständige A0-Protokoll bleibt vor der ersten `0.2.0`-Alpha erforderlich.
