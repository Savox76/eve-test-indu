...
Unter der Bestandsansicht zeigt Paket 21 zusätzlich den [nachvollziehbaren Asset-Änderungsverlauf](asset-deltas.md). Er verwendet dasselbe Suchfeld und denselben Besitzerfilter und ergänzt einen Filter nach Änderungsart. Seit Paket 42 startet die Historie in einer serverseitig vor der Pagination gebildeten Gruppenansicht nach Besitzer, Typ und Snapshotvergleich. Jede Gruppe lässt ihre begrenzt geladenen Einzelereignisse aufklappen; alternativ bleibt die vollständige Ereignisansicht wählbar. Historische Standortpfade stehen vor dem rohen ESI-Bereichsflag, sodass `AutoFit` nur noch als sekundärer technischer Nachweis erscheint.
