...
EVE autorisiert jeden Charakter einzeln. Eine lokale Kontogruppe dient nur der vom Nutzer vergebenen Organisation mehrerer Charaktere; sie speichert weder EVE-Accountnamen noch Zugangsdaten. Paket 14 speichert den geprüften Refresh Token pro Charakter im Windows-Anmeldespeicher und rotiert ihn über einen verifizierten Zwischen-Slot, ohne den letzten aktiven Wert vorzeitig zu verwerfen. Access Tokens bleiben ausschließlich im Prozessspeicher. Tokenwerte werden niemals in SQLite, API-Antworten oder Logs geschrieben; ein fehlender Anmeldespeicher führt zu einem geschlossenen Fehler statt zu einer Klartext-Ausweichablage. Die globale Schriftgrößenstufe wird als nicht geheime Einstellung in der bestehenden Tabelle `app_settings` gespeichert.
