...
- **In Arbeit:** 22 – Der Gerätetest bestätigte Installer, Betrieb ohne Administratorrechte, Einzelinstanz und Release-Erkennung. `v0.0.5-preview.19` enthält die Neustartkorrektur aus `.17`, die Abnahmenacharbeit aus `.18` sowie eine nicht selbstentpackende Sidecar-Paketierung mit verbindlichem Microsoft-Defender-Scan. Der Windows-CI-Test verlangt Schließen, vollständiges Prozessende und einen zweiten erfolgreichen Start mit demselben `data`-Ordner. Das vollständige A0-Protokoll bleibt vor der ersten `0.2.0`-Alpha erforderlich.
