Warning: truncated output (original token count: 71811)
Total output lines: 5426
  ...
  if (runtimeStatus.state === "ready" && runtimeStatus.sidecar === "error") {
    switch (runtimeStatus.errorCode) {
      ...
      case "sidecar-not-found":
        runtimeD…21811 tokens truncated… loadSkills={loadSkills}
    ...
    try {
      await savePlan({ planId: item.planId, ownerCharacterId: draft.owner,
        ...
        stepBlueprintAssignments: item.steps.slice(0, -1).flatMap((step) => {
          ...
          const blueprintItemId = draft.stepBlueprintItemIds[
            `${step.blueprintTypeId}:${step.activity}:${step.productTypeId}`
          ];
