...
Diese Abnahme ist der letzte externe Nachweis vor der ersten `0.2.0`-Alpha. Sie wird auf einem freigegebenen Windows-10- oder Windows-11-x64-Testgerät mit einer vorhandenen Preview und dem Korrekturkandidaten `v0.0.5-preview.16` durchgeführt. Der CI-Pakettest installiert und startet den Kandidaten inzwischen wirklich; den vollständigen Gerätetest ersetzt er dennoch nicht.
...
| 5 | Installer-Update | Nach vollständigem Schließen wird `v0.0.5-preview.16` über `.15` installiert. Die Datenlöschung bleibt abgewählt; anschließend sind Version, Charaktere, Einstellungen, Pläne und Snapshots vorhanden und `data` liegt neben der EXE. |
