...
- **In Arbeit:** 22 – Der Gerätetest bestätigte Installer, Betrieb ohne Administratorrechte, Einzelinstanz und Release-Erkennung. `v0.0.5-preview.22` enthält zusätzlich zur Defender-geprüften GUI-/Sidecar-Grundlage die Korrektur für positive ESI-Blueprint-Stapelmengen, die Frontend-Prüfung mehrstufiger Produktionspläne und kompaktere Charakter- sowie Asset-Bedienung. Der Windows-CI-Test prüft den GUI-Subsystemwert im PE-Header, beendet den Sidecar absichtlich, verlangt einen neuen Prozess, bricht verwaiste Sync-Läufe sicher ab und prüft danach Schließen, vollständiges Prozessende und einen zweiten erfolgreichen Start mit demselben `data`-Ordner. Das vollständige A0-Protokoll bleibt vor der ersten `0.2.0`-Alpha erforderlich.
