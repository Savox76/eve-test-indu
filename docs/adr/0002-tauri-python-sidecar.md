...
- Die Windows-Release-EXE verwendet das GUI-Subsystem. Es gibt neben der WebView kein separat schließbares Konsolenfenster, das den Elternprozess hart beenden und dessen Shutdown-Pfad umgehen könnte.
...
Architektur-Gate A0 muss auf Windows Start ohne zusätzliches Konsolenfenster, GUI-Subsystem im PE-Header, dynamischen Port, Ablehnung ohne beziehungsweise mit falschem Token, paralleles SQLite-Lesen, kontrollierten Shutdown, erzwungenen Sidecar-Abbruch mit automatischer Wiederherstellung und Installation beweisen. Scheitert das Gate, wird diese ADR ersetzt, bevor Fachmodule auf dem Sidecar aufbauen.
