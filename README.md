...
**Technische Preview – `v0.0.5-preview.5`.** Der zentrale ESI-Client ist jetzt mit dem ersten echten Fachsync und der ersten echten Fachansicht verbunden: Typen, Gruppen und Orte können als atomarer SDE-Bestand aufgebaut werden, aktivierte Charaktere werden beim Programmstart vollständig paginiert und charaktergetrennt synchronisiert, Standort- und Containerpfade werden belastbar aufgelöst und die Asset-Oberfläche macht diese vollständigen Snapshots einschließlich nachvollziehbarer Änderungen durchsuchbar, filterbar und sortierbar. Nur vollständig abgeschlossene Läufe werden als gültiger Snapshot oder Delta-Nachweis veröffentlicht.

Charakteridentitäten, Aliasse, Gruppen und Scope-Status stammen weiterhin aus SQLite; Refresh Tokens bleiben im Windows-Anmeldespeicher und Access Tokens ausschließlich im Sidecar-Prozess. Beim Start führt die App den echten ESI-Asset-, Typnamen- und Standortlauf für alle aktivierten Charaktere im Hintergrund aus; **Assets aktualisieren** bleibt für einen manuellen Folgelauf verfügbar. Installierte Ausgaben speichern ihre Daten updatefest im benutzerspezifischen App-Datenordner; portable Ausgaben behalten den sichtbaren `data`-Ordner im konstanten Verzeichnis `New Eden Foundry Portable`. `v0.0.5-preview.5` ist der aktuelle Kandidat für die [verbindliche Windows-A0-Abnahme](docs/windows-a0-acceptance.md).

Registrierungswerte stehen im [verbindlichen Registrierungsprofil](docs/sso-registration.md); die Betriebsgrenzen beschreiben die [PKCE-Dokumentation](docs/sso-pkce-login.md), die [Tokenprüfung](docs/sso-token-validation.md), die [Tokenablage](docs/token-security.md), die [Charakterverwaltung](docs/character-management.md), der [zentrale ESI-Client](docs/esi-client.md), der [Asset-Sync](docs/asset-sync.md), die [Standortauflösung](docs/location-resolution.md), die [Asset-Oberfläche mit CSV-Export](docs/asset-ui.md) und die [Asset-Deltas](docs/asset-deltas.md). Die Anwendung enthält kein Client Secret und gibt weder Tokens noch `state`, Verifier oder Autorisierungscode an Oberfläche oder Logs weiter.
